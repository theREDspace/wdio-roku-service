 ...
 */
export const installByID = async (
    channelId: string
  ): Promise<boolean> => {
    try {
...
export const installFromZip = async (pathToArchive: string): Promise<boolean> => {
      ...
      const response: Response = await ECP(
        endpoints["load"],
        "POST",
        false,
        form,
...
const getDigestAuth = (digest: Map<string, string>) => {
  ...
  const response = getMD5(
    `${hash1}:${digest.get("nonce")}:${nonceCount}:${cnonce}:${digest.get(
      "qop"
    )}:${hash2}`
  );
  return `Digest username="${digest.get("username")}", realm="${digest.get(
    ...
    "uri"
  )}", cnonce="${cnonce}", nc=${nonceCount}, qop="${digest.get(
    "qop"
  )}", response="${response}"`;
};
